...
function ChatPageContent(): React.JSX.Element {
  ...
  const [mobilePromptChipsOpen, setMobilePromptChipsOpen] = useState(false);
  ...
  useEffect(() => {
    if (hasSentMessage) {
      setMobilePromptChipsOpen(false);
    }
  }, [hasSentMessage]);
  ...
  useEffect(() => {
    if (!composerFocused || !mobilePortfolioOpen) return;
    setMobilePortfolioOpen(false);
  }, [composerFocused, mobilePortfolioOpen]);
  ...
  return (
    <main className="h-[100dvh] overflow-x-hidden overflow-y-hidden bg-[var(--bg-primary)] text-[var(--text-primary)]">
      <div className="flex h-full">
        <aside className="gentle-enter hidden h-full w-[260px] shrink-0 border-r border-[var(--border-subtle)] bg-[var(--bg-secondary)] lg:block">
          <SidebarPanel
            profile={profile}
        ...
        <section className="gentle-enter relative flex min-h-0 min-w-0 flex-1 flex-col bg-[var(--bg-primary)]">
          <header className="flex h-[52px] items-center justify-between border-b border-[var(--border-subtle)] px-3 sm:px-4 md:px-6">
            <p className="mono min-w-0 flex-1 truncate pr-2 text-[12px] text-[var(--text-secondary)] sm:text-[13px]">{contextLabel}</p>
            <div className="flex shrink-0 items-center gap-1 sm:gap-2">
              ...
              <button
                ...
                className="whitespace-nowrap text-[12px] text-[var(--text-tertiary)] transition-colors duration-150 hover:text-[var(--text-primary)] sm:text-[13px]"
              >
                New chat
              </button>
            </div>
          ...
          <div ref={messagesViewportRef} className="hide-scrollbar min-h-0 flex-1 overflow-x-hidden overflow-y-auto">
            <div className="mx-auto w-full max-w-[720px] px-3 py-5 sm:px-4 sm:py-6 md:px-6">
              {messages.length === 0 ? (
                <div className="fade-in flex min-h-[58vh] flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-3 text-[12px] text-[var(--text-tertiary)]">
                    Use Suggested prompts above the input to get started.
                  </p>
                </div>
              ) : (
                <div>
          ...
          <div
            className={`gentle-enter-delayed shrink-0 border-t border-[var(--border-subtle)] bg-[var(--bg-primary)] px-3 py-4 sm:px-4 md:px-6 ${
              composerFocused ? "mb-0" : "mb-[calc(64px+env(safe-area-inset-bottom))] lg:mb-0"
            }`}
          >
            <div className="mx-auto w-full max-w-[720px]">
              <div
                className={`overflow-hidden transition-all duration-200 ${
                  hasSentMessage ? "mb-0 max-h-0 opacity-0" : "mb-3 max-h-[360px] opacity-100"
                }`}
              >
                {!hasSentMessage && (
                  <>
                    <div className="sm:hidden">
                      <button
                        type="button"
                        aria-expanded={mobilePromptChipsOpen}
                        aria-controls="mobile-prompt-chips"
                        onClick={() => setMobilePromptChipsOpen((prev) => !prev)}
                        className="inline-flex items-center gap-1 text-[12px] font-medium text-[var(--text-secondary)] transition-colors duration-150 hover:text-[var(--text-primary)]"
                      >
                        Suggested prompts
                        <ChevronDown
                          className={`h-3.5 w-3.5 transition-transform duration-200 ${
                            mobilePromptChipsOpen ? "rotate-180" : "rotate-0"
                          }`}
                        />
                      </button>

                      <div
                        id="mobile-prompt-chips"
                        className={`overflow-hidden transition-all duration-200 ${
                          mobilePromptChipsOpen ? "mt-2 max-h-56 opacity-100" : "max-h-0 opacity-0"
                        }`}
                      >
                        <div className="hide-scrollbar -mx-1 flex snap-x gap-2 overflow-x-auto px-1 pb-1">
                          {PROMPTS.map((prompt) => (
                            <button
                              key={`mobile-input-${prompt}`}
                              type="button"
                              onClick={() => void handleSend(prompt)}
                              className="max-w-[85vw] snap-start rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--bg-primary)] px-3 py-2 text-left text-[13px] text-[var(--text-secondary)] transition-colors duration-150 hover:border-[var(--border-strong)] hover:bg-[var(--bg-secondary)]"
                            >
                              {prompt}
                            </button>
                          ))}
              ...
              >
                <div className="rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--bg-secondary)] px-4 py-3 transition-colors duration-150 focus-within:border-[var(--border-strong)]">
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2 sm:gap-3">
                    <p
                      className={`min-w-0 flex-1 truncate text-[12px] italic text-[var(--text-tertiary)] transition-opacity duration-200 ${
      ...
      <nav
        className={`fixed bottom-0 left-0 right-0 z-30 border-t border-[var(--border-subtle)] bg-[var(--bg-primary)]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur supports-[backdrop-filter]:bg-[var(--bg-primary)]/85 transition-transform duration-200 lg:hidden ${
          composerFocused ? "pointer-events-none translate-y-full" : "translate-y-0"
        }`}
      >
        <div className="mx-auto grid h-[64px] w-full max-w-[720px] grid-cols-3 gap-1 px-2 py-2">
          <button
            type="button"
            onClick={() => setMobilePortfolioOpen(false)}
            className={`rounded-[var(--radius-md)] text-[13px] transition-colors duration-150 ${
              !mobilePortfolioOpen
          ...
          <button
            ...
            className="rounded-[var(--radius-md)] text-[13px] text-[var(--text-tertiary)] transition-colors duration-150 hover:text-[var(--text-primary)]"
          >
            Signals
          </button>
        </div>
      ...
      {mobilePortfolioOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          <button
            ...
            className="overlay-fade-in absolute inset-0 bg-black/10"
            onClick={() => setMobilePortfolioOpen(false)}
          />
          <div className="drawer-slide-up absolute bottom-[calc(64px+env(safe-area-inset-bottom))] left-0 right-0 h-[60vh] rounded-t-[var(--radius-lg)] border-t border-[var(--border-subtle)] bg-[var(--bg-primary)]">
